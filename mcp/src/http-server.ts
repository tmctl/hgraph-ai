...
app.post(`${apiPrefix}/batch`, async (req: Request, res: Response) => {
  try {
    ...
    const responses = await Promise.all(requests.map((request) => processJSONRPCRequest(request)));
...
async function main() {
  ...
  app.listen(port, host, () => {
    ...
    console.error(`Authorization: ${AUTH_ENABLED ? 'Enabled (Bearer token)' : 'Disabled'}`);
    console.error('================================================');
  });
}
